import { promesa } from "./utils.js";
import { location } from "./searchLocation.js";

let conteiner = document.querySelector("#conteinerCards")

async function arrayInfo() {
    ...
    <div class="flex justify-between py-6 items-center">
      ...
      <h1 class="text-4xl font-bold">Stay in ${respuesta[0].country}</h1>
      <!--cantidad de estancias en el país-->
      <p class="text-gray-700">${respuesta.length}+ stays</p>
    </div>
  ...
  `;
    ...
    respuesta.forEach(({ beds, photo, rating, superHost, title, type }) => {
                section.innerHTML += `
